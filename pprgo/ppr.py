...
@numba.njit(cache=True, parallel=True)
def calc_ppr_l1_parallel(indptr, indices, deg, alpha, epsilon, nodes):
    js = [np.zeros(0, dtype=np.int64)] * len(nodes)
    vals = [np.zeros(0, dtype=np.float32)] * len(nodes)
    for i in numba.prange(len(nodes)):
        j, val = _calc_ppr_node(nodes[i], indptr, indices, deg, alpha, epsilon)
        j_np, val_np = np.array(j), np.array(val)
        # idx = np.argwhere(val_np>0).reshape(-1)  # get index where value>0
        # js[i] = j_np[idx]
        # vals[i] = val_np[idx]
        js[i] = j_np
        vals[i] = val_np
...
def ppr_l1(adj_matrix, alpha, epsilon, nodes):
    """Calculate the PPR matrix approximately using Anderson."""

    out_degree = np.sum(adj_matrix > 0, axis=1).A1
    nnodes = adj_matrix.shape[0]

    neighbors, weights = calc_ppr_l1_parallel(adj_matrix.indptr, adj_matrix.indices, out_degree,
                                                numba.float32(alpha), numba.float32(epsilon), nodes)
...
def l1_ppr_matrix(adj_matrix, alpha, eps, idx, normalization='row'):
    """Create a sparse matrix where each node has up to the topk PPR neighbors and their weights."""

    matrix = ppr_l1(adj_matrix, alpha, eps, idx).tocsr()
    ...
    if normalization == 'sym':
        ...
        # assert np.all(deg[idx[row]] > 0)
        # assert np.all(deg[col] > 0)
        matrix.data = deg_sqrt[idx[row]] * matrix.data * deg_inv_sqrt[col]
    elif normalization == 'col':
        # Assume undirected (symmetric) adjacency matrix
        deg = adj_matrix.sum(1).A1
        deg_inv = 1. / np.maximum(deg, 1e-12)
        ...
        # assert np.all(deg[idx[row]] > 0)
        # assert np.all(deg[col] > 0)
        matrix.data = deg[idx[row]] * matrix.data * deg_inv[col]
    elif normalization == 'row':
        pass
    else:
        raise ValueError(f"Unknown PPR normalization: {normalization}")
